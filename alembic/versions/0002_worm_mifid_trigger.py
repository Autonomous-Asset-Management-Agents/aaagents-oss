...
def upgrade() -> None:
    ...
    if os.environ.get("ENABLE_WORM_L2_RBAC", "false").lower() == "true":
        # WARNING: The previous hardcoded password 'app_user_trading_2026!' is compromised
        # in the Git history. Use git filter-repo before production release if this repository
        # becomes public, or ensure this environment variable is strictly controlled.
        app_password = os.environ.get("APP_USER_PASSWORD")
        if not app_password:
            raise ValueError(
                "APP_USER_PASSWORD environment variable must be set when ENABLE_WORM_L2_RBAC is true"
            )
        op.execute(f"CREATE USER app_user WITH PASSWORD '{app_password}';")
        op.execute(
