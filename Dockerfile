# Dockerfile for AI Trading Bot (Engine, API, Web UI)
...
# Run GCS sync, apply DB migrations (non-blocking), then start engine.
# alembic upgrade head: creates round_table_sessions + all missing tables (MiFID II compliance).
# '|| echo' ensures migration failure does NOT block the engine from starting.
CMD ["/bin/bash", "-c", "python scripts/gcs_sync_on_start.py && if [ -n \"$CLOUD_SQL_CONNECTION_NAME\" ]; then echo 'Waiting for Cloud SQL Proxy socket...'; while [ ! -S /cloudsql/$CLOUD_SQL_CONNECTION_NAME/.s.PGSQL.5432 ]; do sleep 0.5; done; echo 'Socket ready!'; fi && (alembic upgrade head || echo '[WARN] Alembic migration failed — continuing without migration') && python -m core.engine"]
