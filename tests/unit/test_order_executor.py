    ...
    async def test_compliance_check_order_blocks_execution(self):
        ...
        with patch("core.engine.order_executor.RedisClient") as mock_redis:
            mock_redis.get_redis = AsyncMock(
                return_value=MagicMock(publish=AsyncMock(), lock=MagicMock(return_value=MagicMock(acquire=AsyncMock(return_value=True), release=AsyncMock(return_value=True))))
            )
...
class TestZeroSizePublishesRedis:
    ...
    async def test_zero_size_publishes_rejection_event(self):
        ...
        redis_mock.lock = MagicMock(return_value=MagicMock(acquire=AsyncMock(return_value=True), release=AsyncMock(return_value=True)))
