...
async def test_require_defaults_true_off_local(tmp_path, monkeypatch):
    # No explicit TFT_REQUIRE_MANIFEST → strict unless DEPLOYMENT_MODE=LOCAL.
    ...
    assert await _ensure(mr._TFTModelRegistry()) is not None  # LOCAL → permissive
...
async def test_versioned_ds_verified_and_pinned(tmp_path, monkeypatch):
    ...
    assert engine is not None
    assert (
        engine._pinned_ds_path == d / "training_ds_v2.pkl"
    )  # load() uses verified file
