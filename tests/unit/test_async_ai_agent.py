...
async def test_async_ai_agent_threadpool_isolation():
    ...
    class ThreadCheckAgent(AsyncAIAgent):
        ...
        def _run_inference(self, state):
            ...
            return VoteResult("ThreadCheck", state.get("symbol", "AAPL"), 0.9, 1.0, f"is_main_thread={is_main_thread}", not is_main_thread)

    agent = ThreadCheckAgent()
    state = {"symbol": "AAPL"}
    
    result = await agent.vote(state)
    assert result.reasoning == "is_main_thread=False", "Inference must run in a worker thread, not the main thread!"
