"""
Lock-in tests for OSS Tier A hygiene defaults (AAAgents audit, 2026-04-24).

A7 — `aaagents.devv` typo domain swept out of source files
...
# Allow AAAgents_REPO_ROOT env var override (set by oss-ci.yml to ${{ github.workspace }}).
_root_override = os.environ.get("AAAgents_REPO_ROOT")
...
def test_a7_no_devv_typo_in_source() -> None:
    """No source file may contain `aaagents.devv` or `aaagents.devvv`.

    Why this matters: the triple-v domain `aaagents.devvv` is unregistered;
    anyone could buy it for ~10 €/yr and start intercepting disclosure reports,
    partner inquiries, and CI alert emails. The double-v form `aaagents.devv`
    is also disallowed as it is one character off the correct `aaagents.de`
    domain and signals sloppiness to fintech reviewers. Both forms are banned.
    """
    typo_domains = ("aaagents.devv", "aaagents.devvv")
    offenders: list[str] = []
    for f in _iter_source_files():
        ...
        for typo in typo_domains:
            if typo in text:
                offenders.append(f"{f.relative_to(REPO_ROOT)} (contains '{typo}')")
                break  # one offender entry per file is enough
    assert not offenders, (
        "Found typo domain(s) in source files (should be `aaagents.de`):\n  "
        + "\n  ".join(offenders)
    )
