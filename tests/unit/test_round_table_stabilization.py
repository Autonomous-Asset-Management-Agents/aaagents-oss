...
class TestRunnerVoteLogging:
    ...
    async def test_vote_log_emitted_per_agent(self):
        ...
        with patch("core.round_table.runner._active_agents", new=mock_agent_list), patch(
...
class TestRLStrategyLoadTorchModelAssets:
    ...
    def test_weights_only_true_fallback_to_false(self, tmp_path, caplog):
        """
        Gherkin:
          Given: weights_only=True wirft Exception (Inkompatibilität)
          When:  _load_torch_model_assets()
          Then:  Retry mit weights_only=False, WARNING geloggt
        """
        import torch
        ...
        assert (
            call_count[0] >= 2
        ), "torch.load muss mindestens 2x aufgerufen werden (1x weights_only=True, 1x False)"
        warning_logs = [
            r for r in caplog.records if "weights_only" in r.message.lower()
        ]
        assert warning_logs, "⚠️ WARNING über weights_only Fallback muss geloggt werden"
...
class TestRLSignalV5ObservationSpace:
    ...
    def _make_mixin_v5(self):
        ...
        mixin.client.get_position = MagicMock(return_value=None)
        mixin.symbols = ["AAPL"]
...
class TestP2AgentWeightZeroExclusion:
    ...
    async def test_news_sentiment_gemini_unavailable_weight_zero(self):
        ...
        with patch("core.round_table.agents._gemini_module", new=None):
            result = await agent.vote(self._make_state())
        assert result.weight == 0.0, (
            f"Gemini unavailable -> weight=0.0. Got: {result.weight}. "
