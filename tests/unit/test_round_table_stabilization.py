...
class TestRLStrategyLoadTorchModelAssets:
    ...
    def test_weights_only_true_fails_securely(self, tmp_path, caplog):
        ...
        assert call_count[0] == 1, "torch.load darf nur 1x mit weights_only=True gerufen werden"
        
        error_logs = [r for r in caplog.records if "FAILED to load PyTorch model: RuntimeError: Simulated weights_only=True incompatibility" in r.message]
        assert error_logs, f"❌ ERROR-Log erwartet, gefunden: {[r.message for r in caplog.records]}"
