...
class TestRedirectBypassPrevention:
    """SR1 — HTTP redirects must be re-validated against the allow-list.

    Originally enforced by ``_NoRedirectOpener`` (block-all). Replaced
    2026-05-07 with ``_AllowlistedRedirectOpener`` after the senior-engineering
    review caught that block-all rejected GitHub's normal 302→CDN redirect
    chain (every Release download fails). The new opener:

    * Re-validates each redirect target against ``_ALLOWED_URL_PREFIXES`` —
      preserves the SR1 security property (no IMDS / localhost bypass).
    * Allows redirects within the allow-list (e.g. ``github.com/...`` →
      ``release-assets.githubusercontent.com/...``) so legitimate downloads
      succeed.
    """
    ...
    def test_redirect_raises_url_error_and_file_not_written(
        ...
        """A 301/302 redirect from a valid GitHub URL to a NON-allow-listed
        host (e.g. internal IMDS) must raise ``URLError`` and the file must
        not be written.

        We simulate urllib's redirect mechanism by making urlopen raise a
        ``urllib.error.URLError`` (which is what ``_AllowlistedRedirectOpener``
        produces when the target is outside the allow-list) and verify the
        ...
        def _redirect_then_fail(request_or_url, *a, **kw):
            # Simulate _AllowlistedRedirectOpener raising on a redirect to a
            # non-allow-listed target (e.g. http://169.254.169.254 IMDS).
            raise urllib.error.URLError(
                "Security policy: HTTP redirect target not in allow-list"
            )
...
# ---------------------------------------------------------------------------
# SR1 (2026-05-07 update): allow-listed redirects MUST be followed.
#
# The old _NoRedirectOpener blocked all redirects, which broke GitHub Release
# downloads (every release URL 302s to release-assets.githubusercontent.com).
# The new _AllowlistedRedirectOpener re-validates each redirect target against
# _ALLOWED_URL_PREFIXES — preserving the SR1 security property while allowing
# legitimate CDN redirects.
# ---------------------------------------------------------------------------


class TestAllowlistedRedirectOpener:
    """Direct unit tests for _AllowlistedRedirectOpener.redirect_request."""

    def _make_handler_request(self, mod, original_url: str):
        """Build a (handler, req, fp, code, msg, headers) tuple for the
        ``redirect_request`` signature without actually opening a connection.
        """
        import io
        from email.message import Message

        handler = mod._AllowlistedRedirectOpener()
        req = urllib.request.Request(original_url)
        # `fp` and `headers` are passed by urllib's HTTPRedirectHandler when
        # processing a 3xx response; we provide minimal stand-ins so the
        # parent-class call path (for the allow-listed case) does not crash.
        fp = io.BytesIO(b"")
        headers = Message()
        return handler, req, fp, headers

    def test_redirect_to_allow_listed_release_assets_cdn_is_followed(self):
        """github.com/.../releases/download/... → release-assets.githubusercontent.com
        is the canonical GitHub Release redirect chain. Must be allowed.
        """
        mod = _load_module()
        handler, req, fp, headers = self._make_handler_request(
            mod, "https://github.com/o/r/releases/download/v/m.bin"
        )
        new = (
            "https://release-assets.githubusercontent.com/"
            "github-production-release-asset/123/abc?token=xyz"
        )
        # super().redirect_request() returns a Request for the new URL.
        result = handler.redirect_request(req, fp, 302, "Found", headers, new)
        assert isinstance(result, urllib.request.Request)
        assert result.get_full_url() == new

    def test_redirect_to_imds_raises_url_error(self):
        """169.254.169.254 (AWS IMDS) is not in the allow-list — must raise."""
        mod = _load_module()
        handler, req, fp, headers = self._make_handler_request(
            mod, "https://github.com/o/r/releases/download/v/m.bin"
        )
    ...
    def test_redirect_to_localhost_admin_raises_url_error(self):
        """http://localhost:8080/admin is not in the allow-list — must raise."""
        mod = _load_module()
        handler, req, fp, headers = self._make_handler_request(
            mod, "https://github.com/o/r/releases/download/v/m.bin"
        )
        with pytest.raises(urllib.error.URLError):
            handler.redirect_request(
                req, fp, 302, "Found", headers, "http://localhost:8080/admin"
            )

    def test_redirect_to_file_scheme_raises_url_error(self):
        """file:///etc/passwd is not in the allow-list — must raise."""
        mod = _load_module()
        handler, req, fp, headers = self._make_handler_request(
            mod, "https://github.com/o/r/releases/download/v/m.bin"
        )
        with pytest.raises(urllib.error.URLError):
            handler.redirect_request(
                req, fp, 302, "Found", headers, "file:///etc/passwd"
            )

    def test_redirect_to_objects_githubusercontent_is_followed(self):
        """objects.githubusercontent.com is preserved in the allow-list for
        legacy assets — must still be allowed.
        """
        mod = _load_module()
        handler, req, fp, headers = self._make_handler_request(
            mod, "https://github.com/o/r/releases/download/v/m.bin"
...
class TestAllowlistContents:
    """Lock-in for the URL allow-list contents.

    Surfaced by 2026-05-07 review: the missing
    ``release-assets.githubusercontent.com`` entry made every GitHub Release
    download fail, even though the manifest URLs were correctly formed.
    """

    def test_release_assets_cdn_is_allow_listed(self):
        mod = _load_module()
        assert any(
            p == "https://release-assets.githubusercontent.com/"
            for p in mod._ALLOWED_URL_PREFIXES
        ), (
            "release-assets.githubusercontent.com MUST be in _ALLOWED_URL_PREFIXES "
            "— GitHub redirects every release-download URL to that host."
        )

    def test_github_dot_com_is_allow_listed(self):
        mod = _load_module()
        assert "https://github.com/" in mod._ALLOWED_URL_PREFIXES

    def test_no_http_or_file_or_ftp_in_allow_list(self):
        mod = _load_module()
        for prefix in mod._ALLOWED_URL_PREFIXES:
            assert prefix.startswith("https://"), (
                f"Allow-list must be HTTPS-only. Found: {prefix!r}"
            )
