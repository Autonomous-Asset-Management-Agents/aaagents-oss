...
def test_a4_firebase_enabled_flag_defined(firebase_text: str) -> None:
    assert "FIREBASE_ENABLED" in firebase_text, (
        "firebase.ts must define a FIREBASE_ENABLED gate driven by VITE_ENABLE_FIREBASE."
    )
    assert 'VITE_ENABLE_FIREBASE' in firebase_text, "VITE_ENABLE_FIREBASE env var must be referenced"
    assert '=== "true"' in firebase_text, (
...
def test_a9_frontend_bound_to_loopback(compose_text: str) -> None:
    assert '"127.0.0.1:80:8080"' in compose_text, (
        "Frontend must bind to 127.0.0.1:80 by default — exposing HTTP on 0.0.0.0 "
        "leaks Firebase auth tokens and broker API headers in cleartext."
    )
    # And no public bind anywhere in the OSS compose
    public_binds = [
        line for line in compose_text.splitlines()
        if '"80:8080"' in line and "127.0.0.1" not in line
    ]
    assert not public_binds, f"Found unscoped 80:8080 bind in OSS compose: {public_binds}"


def test_a9_troubleshooting_has_tls_guidance() -> None:
    text = TROUBLESHOOTING.read_text(encoding="utf-8")
    assert "Caddy" in text, "TROUBLESHOOTING.md must include a Caddy reverse-proxy example for TLS"
    assert "reverse_proxy 127.0.0.1:80" in text, (
