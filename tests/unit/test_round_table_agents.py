...
class TestRedisWeightInjectionDetection:
    ...
    def test_out_of_bounds_redis_weight_triggers_security_warning(self, caplog):
        """
        Given: Redis returns 99999.0 for NewsSentimentAgent (max_weight=35.0)
        When:  agent.weight property is accessed
        Then:  SECURITY WARNING log is emitted with 'rogue' or 'manipulation' context
        And:   Returned weight == max_weight (35.0), NOT 99999.0
        """
        import logging
        from unittest.mock import MagicMock, patch
    ...
    def test_out_of_bounds_low_redis_weight_triggers_security_warning(self, caplog):
        """
        Given: Redis returns -5.0 for LSTMSignalAgent (min_weight=15.0)
        When:  agent.weight property is accessed
        Then:  SECURITY WARNING log is emitted
        And:   Returned weight == min_weight (15.0), NOT -5.0
        """
        import logging
        from unittest.mock import MagicMock, patch
    ...
    def test_in_bounds_redis_weight_no_warning(self, caplog):
        """
        Given: Redis returns 20.0 for LSTMSignalAgent (bounds: [15.0, 40.0])
        When:  agent.weight property is accessed
        Then:  No SECURITY WARNING emitted, returned weight == 20.0
        """
        import logging
        from unittest.mock import MagicMock, patch
        ...
        agent = LSTMSignalAgent()
        mock_redis = MagicMock()
        mock_redis.hget.return_value = b"20.0"
        ...
        assert (
            actual_weight == 20.0
        ), f"In-bounds weight 20.0 should pass through, got {actual_weight}"
