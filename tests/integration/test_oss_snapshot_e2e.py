...
# Repo root: this file is at AI Trading Bot/tests/integration/test_oss_snapshot_e2e.py
...
def test_shadow_boot_paper_trading_fallback_token(snapshot_dir: Path) -> None:
    ...
    `AI Trading Bot/scripts/shadow_boot.py` (the *private* file) instead
    ...
    assert "PAPER_TRADING" in payload, (
        ...
        "applied to `AI Trading Bot/scripts/shadow_boot.py` (the private "
        "file), but the OSS payload is `scripts/shadow_boot.oss.py` "
        "(renamed to `shadow_boot.py` at snapshot time).\n"
        "Fix: edit `AI Trading Bot/scripts/shadow_boot.oss.py`, not the "
