...
# Path discovery
# ---------------------------------------------------------------------------
# This file: AI Trading Bot/tests/integration/test_oss_install_to_first_trade.py
# parents:   [integration, tests, AI Trading Bot, REPO_ROOT]
REPO_ROOT = Path(__file__).resolve().parents[3]
ENGINE_ROOT = REPO_ROOT / "AI Trading Bot"
        ...
        if path == "/v2/account":
            ...
            snap = {
                sym: {
                    ...
                    "minuteBar": {
                        "o": 151.0, "h": 152.5, "l": 150.8, "c": 152.0,
                        "v": 1_000_000, "t": "2026-03-22T15:00:00Z",
                    },
                    "dailyBar": {
                        "o": 150.0, "h": 155.0, "l": 148.0, "c": 152.0,
                        "v": 50_000_000, "t": "2026-03-22T00:00:00Z",
                    },
                    "prevDailyBar": {
                        "o": 145.0, "h": 151.0, "l": 144.0, "c": 150.0,
                        "v": 48_000_000, "t": "2026-03-21T00:00:00Z",
                    },
                }
...
class TestManifestSchema:
    ...
    def test_manifest_schema_well_formed(self) -> None:
        ...
        assert isinstance(manifest["release_tag"], str) and manifest["release_tag"], (
            f"release_tag must be a non-empty string, got {manifest['release_tag']!r}"
        )
        assert "models" in manifest, "manifest missing models[]"
        assert isinstance(manifest["models"], list) and manifest["models"], (
        ...
        for i, model in enumerate(manifest["models"]):
            ...
            assert sha256_re.match(model["sha256"]), (
            ...
            assert model["url"].startswith(("http://", "https://")), (
                f"{ctx} url must be absolute http(s), got {model['url']!r}"
            )
            assert model["filename"] and "/" not in model["filename"], (
...
class TestEngineBootstrap:
    ...
    def test_engine_modules_import_clean(self, monkeypatch: pytest.MonkeyPatch,
                                          mock_alpaca: str) -> None:
        ...
        monkeypatch.setenv("PROXY_ENGINE_SHARED_SECRET", "smoke-shared-secret-32-chars-long!")
...
class TestRoundTableHappyPath:
    ...
    def test_run_round_table_does_not_crash(self,
        ...
        state = {
            ...
            "ohlc": {
                ...
                "close": 152.0, "volume": 1_000_000.0,
            },
            "market_data_keys": [],
