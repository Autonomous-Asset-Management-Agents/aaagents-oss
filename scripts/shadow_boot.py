...
async def _check_alpaca() -> bool:
    ...
    api_secret_str = config.API_SECRET.get_secret_value() if hasattr(config.API_SECRET, "get_secret_value") else config.API_SECRET
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                logger.info("Alpaca API reachable and Auth valid (%s).", config.BASE_URL)
                return True
                ...
                logger.error("Alpaca check failed: HTTP %s - %s", resp.status_code, resp.text)
                return False
...
async def _check_gemini() -> bool:
    ...
    if not config.GEMINI_API_KEY:
        ...
        paper_trading_env = os.environ.get("config.PAPER_TRADING", str(config.PAPER_TRADING))
    ...
    api_key_str = config.GEMINI_API_KEY.get_secret_value() if hasattr(config.GEMINI_API_KEY, "get_secret_value") else config.GEMINI_API_KEY
