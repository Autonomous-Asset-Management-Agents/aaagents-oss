...
class HistoricalDataProvider:
    ...
    def get_batch_data(
        ...
        cache_file_name = f"batch_{end_date_str}_{days}d_{len(symbols)}s.parquet"
        ...
        if os.path.exists(cache_file_path):
            ...
            try:
                ...
                results = {
                    ...
                    for symbol in master_df.index.get_level_values("symbol").unique()
                }
                logging.info(
        ...
        try:
            ...
            if results:
                ...
                try:
                    pd.concat(results, names=["symbol", "timestamp"]).to_parquet(cache_file_path)
