        ...
        if client_host:
            try:
                ...
                if not (ip.is_private or ip.is_loopback):
                    logger.warning(f"Rejected external access attempt from {client_host} in LocalMockAuth.")
                    raise HTTPException(status_code=403, detail="External access forbidden. Must use a reverse proxy.")
            except ValueError:
