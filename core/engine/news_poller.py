...
class NewsPollerMixin:
    ...
    def _check_proactive_rules(self, article: dict):
        if self.is_simulation:
            return
        active_rules = self.ai_rules.get_rules()
        try:
            current_equity = (
                float(self.api.get_account().equity)
                if self.api and self.api.get_account()
                else 100000.0
            )
        except Exception:
        ...
        for rule in active_rules:
            if rule.get("action") == "proactive_signal":
                ...
                try:
                    existing_position = self.api.get_position(signal_ticker)
                ...
                try:
                    ...
                    logging.error("Failed proactive order: %s", e)
                break
        ...
        while self.news_running.is_set() and not shutdown_event.is_set():
            try:
                ...
                url = (
                    ...
                    f"&published_utc.lte={end_str}&limit=50&apiKey={config.POLYGON_API_KEY}"
                )
