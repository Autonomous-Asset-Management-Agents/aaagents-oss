...
class OrderExecutorMixin:
    ...
    async def _execute_tenant_order(self, tenant: Dict[str, Any], event: SignalEvent):
        ...
        try:
            ...
            client_order_id = client_order_id if isinstance(client_order_id, str) else str(uuid.uuid4())
            req = MarketOrderRequest(
                symbol=symbol, qty=size, side=side_enum, time_in_force=TimeInForce.DAY, client_order_id=client_order_id
            )
            ...
            if action == "SELL":
                try:
                    ...
                    # the broker order was submitted but internal state is now inconsistent.
                    # Risk: duplicate SELL on next cycle. Alert via PubSub.
                    logging.error(
