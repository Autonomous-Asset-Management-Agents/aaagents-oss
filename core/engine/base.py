...
class BotEngine(
    ...
    async def _check_redis(self) -> bool:
        ...
        try:
            ...
            logging.warning("Startup check [redis] FAILED: %s", e)
            return False

    async def _check_gemini(self) -> bool:
        """Prüft ob Gemini API erreichbar ist (1 Token generieren)."""
        try:
    ...
    async def _startup_health_check(self) -> None:
        ...
        Kritisch (→ RuntimeError):  redis, gemini
        ...
        logging.info(
            "✅ Startup health check PASSED (redis=✓, gemini=✓, rl_model=%s)",
    ...
    def __init__(
        ...
        if getattr(config, "ENABLE_COMPLIANCE_GUARDIAN", False):
    ...
    def _append_live_equity_to_benchmark(self) -> None:
        ...
        try:
            ...
            logging.debug("Live equity append to benchmark failed: %s", e)
