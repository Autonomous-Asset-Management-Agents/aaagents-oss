...
The wizard uses ``getpass`` for masked input so that API keys are never
visible in the terminal scrollback or shell history.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "command",
        choices=["setup", "status", "delete", "migrate"],
        help="Action to perform",
    )
    args = parser.parse_args()

    # Basic logging for CLI output
