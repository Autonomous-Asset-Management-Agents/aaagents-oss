...
class NewsProcessor:
    ...
    def __init__(self):
        ...
        self.sentiment_model = None  # resolved lazily via get_gemini_instance()
    ...
    def analyze_sentiments_batch(self, headlines: List[str]) -> Dict[str, dict]:
        ...
        _model = self.sentiment_model or get_gemini_instance()
        if not _model:
            neut_result = {
                "sentiment": "neutral",
                "score": 0.0,
    ...
    def get_historical_news(
        ...
        while current_end_date > start_date:
            if (
                self.sentiment_model is None and get_gemini_instance() is None
