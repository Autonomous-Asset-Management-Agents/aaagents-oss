...
def evaluate(symbol: str, model_dir: Path) -> GateResult:
    ...
    has_walkforward = meta.get("walkforward_ic") is not None
    if val_loss is None:
    ...
    if wf_ic is not None:
        if not isinstance(wf_ic, (int, float)):
            return GateResult(False, f"walkforward_ic is not numeric: {wf_ic!r}")
        if wf_ic <= IC_MIN:
            return GateResult(
                False, f"walkforward_ic={wf_ic:.3f} <= threshold {IC_MIN:.2f}"
            )
    elif strict_wf:
        return GateResult(
