...
class TFTPrediction:
    ...
    ``stock_specialist._fetch_ml_prediction()`` at line ~917.
    """
...
class TFTInferenceEngine:
    ...
    def __init__(self, symbol: str, model_dir: Path) -> None:
        ...
        # RF-3: when the model_registry verify gate has SHA-256-verified a specific
        # training_ds, it pins the resolved path here so load() unpickles EXACTLY the
        # verified file — closing the metadata.json-swap TOCTOU (the gate hashes the
        # resolved ds, then load() must not re-resolve to a different, unverified one).
        self._pinned_ds_path: Optional[Path] = None
    ...
    def _resolve_training_ds_path(self) -> Optional[Path]:
        ...
        """
        # RF-3 TOCTOU guard: if the verify gate pinned an already-verified path, use it
        # verbatim — never re-read metadata.json (which is not SHA-covered and could be
        # swapped between the gate's hash and this load to redirect the unpickle).
        if self._pinned_ds_path is not None:
            return self._pinned_ds_path
    ...
    def load(self) -> bool:
        ...
        try:
            with open(ds_path, "rb") as f:
                self._training_ds = pickle.load(f)
        except Exception as exc:
    ...
    def _predict_inner(self, features_df: pd.DataFrame) -> Optional[TFTPrediction]:
        ...
        # `raw` shape: [N, H, Q]. We take the LAST sample (most recent window),
        # AVERAGE across horizon H, and read three quantile indices.
        try:
        ...
        try:
            # 2026-05-11: model output is in the target's original units after
            # pytorch-forecasting's automatic inverse-transform. The target
            # column was the 5-day forward return (already in percent units
            # per the training pipeline). Earlier `× 100` scaling produced
            # absurd values like 311% bull case for AAPL — the model was
            # already in % so the multiplier was a bug. If a future training
            # run uses raw log-return, switch to `* 100.0` and document.
