...
**Fusion note (dormant):** nothing on `main` imports this module yet — the
specialist wiring (`_fetch_ml_prediction`, flag-gated) is a separate brick.
Landing it is import-safe: ``__init__`` only builds empty in-memory containers,
no torch import (pytorch is imported lazily inside ``TFTInferenceEngine.load``)
and no disk access at import time.
...
def _models_root() -> Path:
    """Per-symbol checkpoint root: ``<TFT_MODELS_ROOT>/<SYM>/`` if the env var is
    set, else module-relative ``core/ml/models/``. Configurable so the
    model-provenance epic can point it at provisioned release assets without a
    code change (read each call — cheap, and keeps tests hermetic)."""
...
class _TFTModelRegistry:
    ...
    def _verify_before_load(
        ...
        try:
            ...
            # resolved training_ds (ADR-ML-DS-01). The resolved ds is then PINNED on the
            # engine so load() unpickles this verified file and cannot be redirected by a
            # later metadata.json swap (TOCTOU).
            required = [(model_dir / "checkpoint.pt").resolve()]
            ds_path = engine._resolve_training_ds_path()
            if ds_path is not None:
                required.append(Path(ds_path).resolve())
            ...
            for art in required:
                ...
                if want is None:
                    ...
                    return False
                if not art.exists() or _sha256(art) != want:
                    logger.warning(
            ...
            engine._pinned_ds_path = ds_path  # load() unpickles the verified file only
            return True
        except Exception as exc:
            logger.warning(
