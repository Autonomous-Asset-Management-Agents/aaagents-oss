...
try:
    from core.intelligent_exit import (
        analyze_exit,
        PositionContext as IntelligentPositionContext,
    )
...
class RLExecutionMixin:
    ...
    async def _evaluate_signal(
        ...
        if self.rl_model is not None:
            ...
            action, self._lstm_states[symbol] = self.rl_model.predict(
                state.reshape(1, -1),
                state=last_state,
                episode_start=dones,
                deterministic=True,
    ...
    async def _run_for_symbol_impl(  # noqa: C901
        ...
        if (
            ...
            try:
                old_pos = self.client.get_position(symbol_to_close)
                if old_pos:
                    old_qty = (
                        float(old_pos.qty)
                    ...
                    if old_qty > 0:
                        ...
                        await self._submit_order_safe(symbol_to_close, old_qty, "sell")
                ...
                self.log_thought(f"[{symbol_to_close}] ⚠️ Failed to close for swap: {e}")
        ...
        if signal == "BUY" and not in_pos:
            try:
                ...
                if cash == 0:
                    cash = reg_cash
                if cash <= 0:
                    self.log_thought(f"[{symbol}] ⚠️ BLOCKED - No buying power available.")
                    return
            except Exception as e:
            ...
            if size == 0:
                self.log_thought(f"[{symbol}] ⚠️ Position size is 0 – insufficient cash!")
                return
        ...
        if signal == "BUY" and not in_pos and size > 0:
            ...
            order_success = await self._submit_order_safe(
                symbol, size, "buy", expected_cost=order_cost
            )
            ...
            self.high_water_marks.pop(symbol, None)
            self._entry_time.pop(symbol, None)
            order_success = await self._submit_order_safe(symbol, qty, "sell")
            if order_success:
                if self.portfolio_manager:
