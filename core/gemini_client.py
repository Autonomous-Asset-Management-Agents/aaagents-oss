...
class GeminiModelWrapper:
    ...
    def generate_content(self, prompt: str, max_output_tokens: int = 512) -> str:
        ...
        response = self._client.models.generate_content(
            ...
            config=genai_types.GenerateContentConfig(
                temperature=0.7,
                max_output_tokens=max_output_tokens,  # Cost guard: default 512 tokens
                safety_settings=[
    ...
    async def generate_content_async(
        ...
        if not response.text:
            logging.warning("Gemini returned empty text! Raw response: %s", response)
...
# ---------------------------------------------------------------------------
# Chat functions — all use get_gemini_instance() instead of the module var
# ---------------------------------------------------------------------------
def answer_trading_chat(
    ...
    Returns None if Gemini unavailable or on error.
    """
...
def answer_with_gemini_general(
    ...
    """
    Use Gemini to answer open-ended questions (market trends, earnings, etc.).
    """
    model = get_gemini_instance()
    if not model:
        return None
...
def answer_chat_with_fallback(system_context: str, user_message: str) -> str:
    ...
    if reply:
        return reply
    return "I couldn't generate an answer. Make sure Gemini API is configured for open-ended questions."
