...
def fetch_bars(
    ...
    headers = {"User-Agent": "antigravity/1.15.8"}
    try:
    ...
    try:
        ...
        if _r is not None:
            ...
            _r.set(_CACHE_KEY, _json.dumps(df_reset.to_dict(orient="records")), ex=_CACHE_TTL)
...
def fetch_fundamentals(api_key: str, symbols: List[str]) -> Dict[str, Dict[str, float]]:
    ...
    for symbol in symbols:
        ...
        try:
            ...
            logging.debug("polygon:fundamentals Redis check failed for %s: %s", symbol, _e)
        ...
        try:
            ...
            try:
                ...
                if _r is not None:
                    ...
                    logging.debug("polygon:fundamentals cached %s (%ds TTL)", symbol, _CACHE_TTL)
            except Exception as _e:
                logging.debug("polygon:fundamentals Redis set failed for %s: %s", symbol, _e)
