...
from core.round_table.senate_log import SenateProtocol, DummyAuditLogger, SenateSession, make_session_id, IAuditLogger
...
async def run_round_table(state: "SymbolEvalState") -> "SymbolEvalState":
    ...
    if _consensus_engine is None or _senate is None:
        ...
        return {**state, "error": "Round Table not initialized. Call boot_engine() first."}
    ...
    try:
        vote_results = await asyncio.gather(
            *[
                asyncio.wait_for(
                    agent.vote(state), timeout=_AGENT_VOTE_TIMEOUT_SEC
                )
