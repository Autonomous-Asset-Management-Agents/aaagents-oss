...
class LocalJSONAuditLogger(IAuditLogger):
    ...
    async def log_session(self, session: SenateSession) -> None:
        ...
        # Disk full guard - execute in ensure_future to not block
        asyncio.ensure_future(self._async_log_to_jsonl(session))

    async def _async_log_to_jsonl(self, session: SenateSession) -> None:
        try:
            # Offload CPU/Disk-intensive Task in Thread-Pool
            ...
            logger.warning(f"LocalJSONAuditLogger write failed: {exc}")
