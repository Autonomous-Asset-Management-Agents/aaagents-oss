...
|---|---|---|
| `AI Trading Bot/core/secret_manager_utils.oss.py` | `OAuthSecretManager.save_tokens()` | OAuth token storage is disabled in OSS. Keys come from `.env.oss` exclusively. |
| `AI Trading Bot/core/secret_manager_utils.oss.py` | `OAuthSecretManager.get_tokens()` | Same. No file I/O, no DB write, zero credential surface. |
| `AI Trading Bot/core/user_wallet_store.py` | `UserWalletStore.update_alpaca_keys()` | Writing API keys to the DB is disabled. Use `.env.oss`. |
...
The following FastAPI routes in `AI Trading Bot/serve_public_api.py` return `HTTP 400 Bad Request` intentionally:
...
### 4.3 OSS vs. Enterprise Feature Matrix

| Feature | OSS Edition | Enterprise Edition |
|---|---|---|
| Credential storage | `.env.oss` file (local) | GCP Secret Manager |
| Authentication | LocalMockAuth (127.0.0.1 only) | Firebase Auth |
| API key input | `.env.oss` only — no dashboard UI | Secret Manager integration |
| Float arithmetic | Python `float` (accepted for low-freq) | `Decimal` throughout |
| Multi-tenant DB schema | Present in DB (for future migration) | Fully active |
