...
text = re.sub(r'class GcpJsonFormatter\(logging\.Formatter\):', mixin_code + 'class GcpJsonFormatter(logging.Formatter, SecretMaskMixin):', text)

text = text.replace('return json.dumps(log_entry)', 'json_str = json.dumps(log_entry)\n        return self._mask_string(json_str)')

filter_pattern = r'class SecretMaskFilter\(logging\.Filter\):.*?(?=def setup_logging)'
text = re.sub(filter_pattern, '', text, flags=re.DOTALL)

text = text.replace('handler.setFormatter(logging.Formatter(fmt))', 'handler.setFormatter(SecretMaskFormatter(fmt))')
