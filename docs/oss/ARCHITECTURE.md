...
## 3. Machine Learning Models (No GCP Dependency)

In the Enterprise Edition, models (`.pt` PyTorch files) are synced dynamically from Google Cloud Storage on boot.
In the Community Edition:
- The system is shipped via pre-built Docker containers (`ghcr.io/aaagents/backend`) which *already contain* the models.
- If you run the code natively (without Docker), you must download the models and place them in the `models/` directory manually. 
