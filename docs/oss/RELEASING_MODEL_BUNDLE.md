...
A freshly-cloned `aaagents-oss` repo, after `docker compose --env-file .env.oss -f docker-compose.oss.yml up -d`, must end up with the canonical LSTM and RL model files in `./data/` so `LSTMSignalAgent` (w=0.40) and `RLConfidenceAgent` (w=0.40) produce informed Senate votes instead of the neutral-0.5 fallback.
...
docker compose up
   └─> backend container CMD
         └─> AI Trading Bot/scripts/gcs_sync_on_start.py
...
- `OSS_DEPLOY_TOKEN` secret on Dev-Enviroment with `contents:write` on
  `Autonomous-Asset-Management-Agents/aaagents-oss`. (Same secret already
  used by `publish-oss-snapshot.yml` — no new secret needed.)
- A populated `AI Trading Bot/data/` directory containing the canonical model
...
`AI Trading Bot/data/<file>` on the runner before it computes SHA256s.
Three supported flows, **only one** is needed:

| Flow | When to use | How |
|---|---|---|
| **A. Artifact upload** | A prior training workflow (`train_lstm.py`, `train_rl.py`) ran in CI and uploaded the outputs as a GitHub Actions artifact. | Pass `models_artifact: <artifact-name>` when dispatching the workflow. The workflow runs `actions/download-artifact@v4` and unpacks into `AI Trading Bot/data/`. |
| **B. Self-hosted runner** | The training pipeline writes to a persistent volume mounted on a self-hosted runner. | Re-route the workflow to `runs-on: self-hosted` (one-line edit) and ensure the volume is mounted at `AI Trading Bot/data/`. |
| **C. Local checkout (current default)** | Operator runs the workflow from a checkout where they have already populated `AI Trading Bot/data/` from `aaagents-app` locally. | Untrack-edit `AI Trading Bot/data/` files into the runner volume via `gh run download` from a previous "stage models" workflow, OR (for tournament mode) use `act` to run the workflow against a local checkout. |
...
> snapshots in `aaagents-app/AI Trading Bot/data/` while the training CI is
> still being built out (out of scope for this tournament).
>
> **Legacy filenames (Flow C):** the v1.0 launch source is the operator's
> `aaagents-app/AI Trading Bot/data/` directory, which contains LEGACY-named
...
> aaagents-oss API steps (release create/view/upload + asset re-verify). The
...
- Each entry's `url` resolves on `aaagents-oss` Releases (click through).
...
`AI Trading Bot/data/models_manifest.json` from `main`, and at workflow-end
...
into the OSS snapshot. The manifest then ships into `aaagents-oss` on the
...
The current canonical source is `C:\Users\gapel\aaagents-app\AI Trading Bot\data\` (operator's local box):
...
The local-only `aaagents-app/AI Trading Bot/data/models/` subdirectory
contains 90 booster heads and senator ensembles which the engine *currently
...
| Symptom | Recovery |
|---|---|
| Step 5 created the release but step 6 (verify) failed | `gh release delete <tag> --yes --repo Autonomous-Asset-Management-Agents/aaagents-oss` then re-run. |
| Step 7 (PR open) failed but release exists | The release is already public. Manually create the manifest commit on a new branch and `gh pr create` — the manifest lives at `AI Trading Bot/data/models_manifest.json` in the workflow's runner artefacts under `models_manifest.json` (re-download via `gh run download <run-id>`). |
...
- `AI Trading Bot/scripts/build_models_manifest.py` — manifest builder
  + verifier. New CLI flags `--models-dir` (alias `--from-dir`), `--org`,
  `--dry-run` are all documented in `python build_models_manifest.py --help`.
- `AI Trading Bot/scripts/gcs_sync_on_start.py` — engine-startup downloader.
