...
|---|---|---|
| Deployment | Docker Compose (local-first) | Google Cloud Run (managed SaaS) |
| Database | PostgreSQL 15 (local via Docker) | AlloyDB AI (audit reasoning lake) |
...
| Explainability | SHAP values (local) | SHAP + audit-proof reasoning lake |
...
The entire stack — backend engine, public API proxy, frontend dashboard, PostgreSQL, and Redis — is delivered as a pre-configured multi-container Compose stack. The only local dependencies are **Git** (to download the repository) and **Docker Desktop**.
